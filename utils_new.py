...
class ImprovedNN(nn.Module):
    def __init__(self, mpc_H, mpc_T, O, K):
        super(ImprovedNN, self).__init__()
        input_size = 3  # Global context size
        
        # Dilated convolution with batch normalization and residual connection
    ...
    def forward(self, x):
        global_context, time_series = x[:, :3], x[:, 3:]
        time_series = time_series.unsqueeze(1)  # Conv1D expects [batch_size, channels, seq_len]
        ...
        time_series = time_series.view(time_series.size(0), -1)  # Flatten for fully connected layers
...
def sample_init(BS, dyn, sn=None):
    ...
    v_sample = torch.randint(int(1.0*di), int(1.5*di), (BS,1), generator=gen)/di
...
def sample_init_traj_dist(BS, dyn, traj, num_patches, sn=None):
    ...
    d_sample = torch.clamp(torch.from_numpy(traj_sample[:,1].reshape(-1,1))+torch.randint(int(-.015*di), int(.015*di), (BS,1), generator=gen)/di,-0.175,0.175)
    phi_sample = torch.from_numpy(traj_sample[:,2].reshape(-1,1))+torch.randint(int(-0.005*di), int(0.005*di), (BS,1), generator=gen)/di
    v_sample = torch.clamp(torch.from_numpy(traj_sample[:,3].reshape(-1,1))+torch.randint(int(-0.05*di), int(0.05*di), (BS,1), generator=gen)/di,0.0,1.5)
