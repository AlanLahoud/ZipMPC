...
class CasadiControl():
    def __init__(self, track_coordinates, params):
        ...
        self.max_track_width_perc = 0.70
...
def sample_init_traj_dist(BS, dyn, traj, num_patches, sn=None):
    ...
    d_sample = torch.clamp(torch.from_numpy(traj_sample[:,1].reshape(-1,1))+torch.randint(int(-.04*di), int(.04*di), (BS,1), generator=gen)/di,-0.15,0.15)
...
def q_and_p(mpc_T, q_p_pred, Q_manual, p_manual):
    ...
    #phi
    q[:,:,2] = (q[:,:,2] + q_p_pred[:,:,4]).clamp(e + 0.5)
    p[:,:,2] = p[:,:,2] + q_p_pred[:,:,5]
