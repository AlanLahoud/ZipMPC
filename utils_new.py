...
def sample_init_traj_dist(BS, dyn, traj, num_patches, sn=None):
    ...
    v_sample = torch.clamp(torch.from_numpy(traj_sample[:,3].reshape(-1,1))+torch.randint(int(-0.1*di), int(0.1*di), (BS,1), generator=gen)/di,0.0,1.8)
...
def q_and_p(mpc_T, q_p_pred, Q_manual, p_manual):
    ...
    q = e*torch.ones((mpc_T,BS,11)) + torch.tensor(Q_manual).unsqueeze(1).float()
    p = torch.zeros((mpc_T,BS,11)) + torch.tensor(p_manual).unsqueeze(1).float()
