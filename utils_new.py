...
def sample_init(BS, dyn, sn=None):
    ...
    v_sample = torch.randint(0, int(1.1*di), (BS,1), generator=gen)/di
...
def sample_init_traj_dist(BS, dyn, traj, num_patches, sn=None):
    ...
    d_sample = torch.clamp(torch.from_numpy(traj_sample[:,1].reshape(-1,1))+torch.randint(int(-.03*di), int(.03*di), (BS,1), generator=gen)/di,-0.22,0.22)
