...
#pt = torch.optim.Adam(model.parameters(), lr=0.00003, weight_decay=1e-5)
opt = torch.optim.RMSprop(model.parameters(), lr=0.0005)

control = utils_new.CasadiControl(track_coord, params)
Q_manual = np.repeat(np.expand_dims(np.array([0, 20, 0, 0, 0, 0, 0, 0, 0, 0]), 0), mpc_T, 0)
p_manual = np.repeat(np.expand_dims(np.array([0, 0, 0, 0, 0, -10.0, 0, 0, 0, 0]), 0), mpc_T, 0)
