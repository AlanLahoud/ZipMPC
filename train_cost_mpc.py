...
p_manual = np.repeat(np.expand_dims(np.array([0, 0, 0, 0, 0, -5.0, 0, 0, 0, 0]), 0), mpc_T, 0)
    ...
    # V A L I D A T I O N   (only casadi) 
        with torch.no_grad():
            ...
            for sim in range(mpc_H//mpc_T):
                ...
                u_pred_noise = u_pred_val + eps_dyn*np.random.randn(u_pred_val.shape[0], u_pred_val.shape[1])
                ...
                u_manual_noise = u_manual + eps_dyn*np.random.randn(u_manual.shape[0], u_manual.shape[1])
