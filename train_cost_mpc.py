...
model = utils_new.SimpleNN(mpc_H, n_Q, 4, max_p)
...
Q_manual = np.repeat(np.expand_dims(np.array([0, 0.5, 0.5, 0, 0, 0.01, 0, 0, 0, 0]), 0), mpc_T, 0)
p_manual = np.repeat(np.expand_dims(np.array([0, 0, 0, 0, 0, -0.5, 0, 0, 0, 0]), 0), mpc_T, 0)
