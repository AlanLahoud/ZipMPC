...
class CasadiControl():
    ...
    def mpc_casadi(self,q,p,x0,dx,du, u0=np.array([0.,0.])):
        ...
        tsh = 0.40*self.max_track_width_perc*self.track_width*np.ones((1,N+1))

        barr1 = if_else(x_sym[1,0:N+1] > tsh, exp(1000 * (x_sym[1,0:N+1] - tsh)) - 1, 0)
        barr2 = if_else(x_sym[1,0:N+1] < -tsh, exp(1000 * (-tsh - x_sym[1,0:N+1])) - 1, 0)
